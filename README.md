...
6. **💾 本地持久化 SQLite 与内存级加速 (Persistent SQLite & Memory-Mapped Speed)**  
   知识索引全量持久化于本地 SQLite 数据库（`.vault_index.db`），配备 WAL 日志与 256MB 内存映射（`mmap`）。电脑重启后索引永生不丢；检索基于 `mtime` 毫秒级增量热同步，兼备物理存盘安全与纯内存吞吐速度。
...
| :--- | :--- | :--- |
| **`vault_db.py`** | SQLite 持久化索引引擎 (WAL + 256MB mmap + mtime 增量同步) | 存储管理、冷启动构建与底层高性能持久化支撑 |
